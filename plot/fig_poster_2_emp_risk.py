...
for data in ["mnist"]:
    ...
    fig, ax = plt.subplots(1, 1, figsize=((0.5*8.27*3.0, 0.15*11.69*3.0)))
    ...
    label_bound_dict = {
        "lever": r"Lever et al. (2013)",
        "dziugaite": r"Dziugaite and Roy (2018)",
        "comp": r"Ours: $\omega : h \mapsto 0$",
        "comp_prior": r"Ours: $\omega : h \mapsto \alpha\Risk^{\ell}_{\Scal'}(h)$",
    }
    ...
    fig.savefig(
        "figures/fig_poster_2_.pdf".format(data),
...
ax.set_axis_off()
legend = fig.legend(
    handles, labels, loc='lower center', frameon=False, ncol=2,
    bbox_to_anchor=(-0, -10), handlelength=1)
for line in legend.get_lines():
    line.set_linewidth(6)

os.makedirs("figures/", exist_ok=True)
plt.savefig(
    "figures/fig_poster_2_legend.pdf", bbox_inches="tight", pad_inches=-0.1)
plt.close(fig)
...
os.makedirs("figures/", exist_ok=True)
plt.savefig(
    "figures/fig_poster_2_legend_2.pdf", bbox_inches="tight", pad_inches=-0.1)
plt.close(fig)
