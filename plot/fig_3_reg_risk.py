...
for data in ["mnist", "fashion"]:
    ...
    ax.set_title(r"{} (Corollary 5 with $\frac{{m'}}{{m'+m}}=0.5$)".format(
        data_text_dict[data]))
    ax.set_xlabel(r"Trade-off parameter $\beta$")
    os.makedirs("figures/", exist_ok=True)
    fig.savefig(
        "figures/fig_3_reg_risk_{}_prior.pdf".format(data),
        bbox_inches="tight")
    plt.close(fig)
...
for data in ["mnist", "fashion"]:
    ...
    ax.set_xlim(beta[0], beta[-1])
    ax.set_title(r"{} (Corollary 4 with $\frac{{m'}}{{m'+m}}=0.0$)".format(
        data_text_dict[data]))
    ax.set_xlabel(r"Trade-off parameter $\beta$")
    os.makedirs("figures/", exist_ok=True)
    fig.savefig(
        "figures/fig_3_reg_risk_{}.pdf".format(data), bbox_inches="tight")
    plt.close(fig)
...
plt.axis('off')
os.makedirs("figures/", exist_ok=True)
plt.savefig("figures/fig_3_legend.pdf", bbox_inches="tight", pad_inches=-0.1)
plt.close(fig)
